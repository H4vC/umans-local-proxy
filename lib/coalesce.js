...
// Cached per-message hash (raw uint32). JSON.stringify is the expensive
// part; cache it so a system prompt reused across 50 turns is
// stringified+hashed exactly once. True LRU: on a hit, delete + re-insert
// to move to end (most-recently-used). Map iteration order is insertion
// order, so keys().next() returns the LRU.
function messageHash(message) {
  const json = JSON.stringify(canonicalMessage(message));
  let h = state.messageHashCache.get(json);
  if (typeof h === 'number') {
    // True LRU: delete + re-insert to move to end (most-recently-used).
    state.messageHashCache.delete(json);
    state.messageHashCache.set(json, h);
    return h;
  }
  // Miss, or stale entry from a pre-L4 build (hex string instead of uint32).
  // Recompute: old hex-string entries were produced by the previous chain
  // hashing strategy and are invalid under numeric mixing — drop them.
  h = fnv1a32(json);
  state.messageHashCache.set(json, h);
