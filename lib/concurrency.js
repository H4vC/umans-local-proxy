...
function getEffectiveConcurrency() {
  ...
  // While the burst cooldown is active (an upstream 429 within the last 24h,
  // or re-armed during the 20h reset window), clamp the effective limit to the
  // soft limit so no requests above the soft concurrency are admitted.
  let effectiveLimit = limit;
  if (burstCooldownActive() && softLimit && effectiveLimit != null) {
    effectiveLimit = Math.min(effectiveLimit, softLimit);
  }
  return { concurrent: state.concurrencyCache.concurrent || 0, limit: effectiveLimit, softLimit, overridden };
}
