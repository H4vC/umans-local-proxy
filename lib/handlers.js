...
async function handleConfig(req, res) {
  if (req.method === 'GET') {
    return writeJSON(res, 200, {
      ...
      releaseCooldown: state.config.releaseCooldownRaw,
  ...
  let releaseCooldownMs = state.config.releaseCooldownMs;
  let releaseCooldownRaw = state.config.releaseCooldownRaw;
  ...
  try {
    ...
    if (typeof next.releaseCooldown === 'string' && next.releaseCooldown.trim()) {
      releaseCooldownRaw = next.releaseCooldown.trim();
  ...
  const nextConfig = { ...state.config, apiKey, fileApiKey, sessionCookie, fileSessionCookie, enabledModels, requestTimeout, requestTimeoutRaw, releaseCooldownMs, releaseCooldownRaw, requestLogging, requestLoggingRaw, overrideConcurrency, websearchProvider, proxyApiKeys, fileProxyApiKeys: fileKeys };
...
function handleClearState(req, res) {
  ...
  if (Array.isArray(state.completedSessionOrder)) state.completedSessionOrder.length = 0;
  state.completedSessionHead = 0;
  if (state.sessionsByGroup) state.sessionsByGroup.clear();
  if (state.sessionTimers) {
    for (const t of state.sessionTimers.values()) clearTimeout(t);
...
async function handleRequest(req, res, shutdown, reload) {
  ...
  if ((url.pathname === '/' || url.pathname === '/dashboard') && req.method === 'GET') {
    ...
    return writeText(res, 200, html, 'text/html; charset=utf-8', { 'Cache-Control': 'no-cache' });
  }
