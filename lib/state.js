...
module.exports = {
  ...
  pendingBodyReads: 0,
  throttleWaiters: [],
  // Absolute deadline from upstream Retry-After values. Each new valid header
  // can only extend this local admission barrier; its timer wakes FIFO waiters
  // when the final deadline actually expires.
  retryAfterUntil: 0,
  retryAfterWakeTimer: null,
  // Released-permit cooldown expirations, kept in ascending order so index 0
  // is always the earliest actual expiry.
  releaseCooldowns: [],
  // Windowed (observed, local) pairs for phantom estimation (sluice Plan 003
  // port). Recorded in doFetchUmansUsage, aligned to the moment the provider
  // counted the sessions (held captured before the fetch await). Bounded to
  // PHANTOM_WINDOW samples.
  phantomSamples: [],
  // One-shot timer for the earliest release-cooldown expiry. cooldownWakeAt
  // identifies its deadline so a newly earlier expiry can re-arm it.
  ...
  completedSessionOrder: [], // completion-order ids; stale entries are skipped during eviction
  completedSessionHead: 0,
  sessionTimers: new Map(), // session id → expiration timer; cancelled on eviction or clear-state
  sessionSeq: 0,
  seenModels: new Set(),
