...
// ---- Frame parsing ----
// Returns { type, data, consumed } or null if incomplete.
// type: 'text' | 'ping' | 'pong' | 'close' | null (fragmented/unsupported)
// Returns { type: 'tooLarge' } if the frame exceeds MAX_FRAME_SIZE, so the
// caller can destroy the connection instead of buffering indefinitely.
...
function parseFrame(buf) {
  ...
  if (masked) {
    ...
    for (let i = 0; i < payloadLen; i++) unmasked[i] = data[i] ^ mask[i & 3];
    data = unmasked;
  }
...
//   { socket, queue: [], paused: false, alive: true }
...
function addClient(socket) {
  const client = { socket, queue: [], paused: false, alive: true };
