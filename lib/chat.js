...
async function proxyRequest(req, res, { shape } = {}) {
  ...
  // Runs AFTER the early SSE headers so a cold/expired model-info cache
  // (fetchModelInfo — up to a 10s hang on /models/info) never blocks the
  // client's 200. The snap only mutates payload, consumed when building the
  // upstream body below; headers don't depend on it.
