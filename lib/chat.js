  ...
  try {
    ...
    tap = new ChatTap(session, {
      ...
      onFinalize: (s) => {
        ...
        try { storeStateKey(payload.model, payload.messages || [], s.responseContent, s.groupKey, prefixChain); } catch (err) { logError('storeStateKey (coalesce) failed', err); }
        }
        s.responseContent = '';
      },
    });
    ...
    const upstream = await fetch(upstreamURL(upstreamPath), {
      method: 'POST',
      headers: authHeaders(
        { 'Content-Type': 'application/json', Accept: payload.stream ? 'text/event-stream' : 'application/json', 'Accept-Encoding': 'gzip, deflate, br' },
