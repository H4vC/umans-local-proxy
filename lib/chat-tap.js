  ...
  if (op === 'sync') {
    ...
    if (s && msg.fields) {
      ...
      if (typeof msg.fields.outputTokens === 'number') {
        ...
        if (h > 128) { samples.splice(0, h); s.tpsHead = 0; }
        else if (h) s.tpsHead = h;
      }
    }
  } else if (op === 'final') {
...
function ensureWorker() {
  ...
  try {
    ...
    for (const [id, init] of state.tapActiveTaps) {
      try { w.postMessage({ op: 'init', id, shape: init.shape, model: init.model, stream: init.stream }); } catch {}
    }
    return w;
  } catch {
    state.tapWorkerDead = true;
...
class ChatTap {
  constructor(session, { stream, onFinalize, shape = 'openai' }) {
    ...
    if (session && session.model) {
      ...
      // C8: track this tap so a worker restart can re-init its parser. Without
      // this, a restarted worker has no parser for the id and silently drops
      // all char/token/responseContent telemetry for the rest of the stream.
      state.tapActiveTaps.set(this.id, { shape, model: session.model, stream });
    }
  }
  ...
  _flush() {
    ...
    // a single memcpy pass replaces the prior per-chunk allocUnsafeSlow+set
    // (N allocs + N copies), and transferring one buffer lets the worker feed
    // it directly with no second Buffer.concat copy. The originals are already
    // in flight to res so they can't be transferred — one copy is the floor.
    let total = 0;
    for (let i = 0; i < refs.length; i++) total += refs[i].length;
    const merged = Buffer.allocUnsafeSlow(total);
    let offset = 0;
    for (let i = 0; i < refs.length; i++) { refs[i].copy(merged, offset); offset += refs[i].length; }
    if (!postToWorker({ op: 'chunk', id: this.id, bufs: [merged] }, [merged.buffer])) {
    ...
    if (!state.tapWorkerDead) {
      fields = await new Promise((resolve) => {
        const timer = setTimeout(() => {
          // Guard: a hot reload between onEnd() and this timeout firing can
          // leave state.tapPendingFinals undefined. Without this guard the
          // process crashes, killing the tap worker and losing ALL telemetry
          // (responseContent, outputTokens, firstTokenAt) for every request.
          const pending = state.tapPendingFinals;
          if (pending instanceof Map && pending.get(this.id)?.resolve === resolve) {
...
module.exports = { ChatTap, createTapStream, restartTapWorker };
