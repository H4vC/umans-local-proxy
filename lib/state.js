...
module.exports = {
  ...
  queuedRequests: 0,
  throttleWaiters: [],
  // Released-permit cooldown queue (sluice release-cooldown port): each entry
  // is an epoch-ms when a freed permit becomes reusable. Pruned by
  // coolingDownNow(). Pushed in release order (monotonic), so it stays sorted.
  releaseCooldowns: [],
  ...
  phantomSamples: [],
  // One-shot timer that wakes queued waiters when the earliest cooling permit
  // expires, so a freed-but-resting slot doesn't strand a queued request until
  // the next /usage refresh. unref'd; cleared on schedule + in beforeEach.
