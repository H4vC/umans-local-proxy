...
function finalizeSession(session, status) {
  ...
  setTimeout(() => { deleteSession(session.id); scheduleSessionsBroadcast(); }, state.SESSION_TTL_MS).unref();
...
function getSessionsSnapshot() {
  ...
  const models = [...state.seenModels].map((model) => {
    ...
    return {
      ...
      ttftMsP50: metrics.models[model]?.ttftMsP50 ?? null,
    };
  ...
  return {
    ...
    aggregate: {
      ...
      ttftMsP50: metrics.overall.ttftMsP50,
      models,
    },
  };
}
...
// Proxy-derived p50 metrics from finished sessions' telemetry — the
// end-to-end experience (queue wait + upstream latency) the user actually
// sees, NOT the upstream /status API's internal measurement. Only finished
// sessions contribute; sessions expire after SESSION_TTL_MS so this is a
// recent-window p50, not a 24h aggregate. Surfaced via the sessions
// snapshot aggregate, not /api/umans/status (service health stays upstream-only).
function sessionMetrics() {
  const tpsByModel = new Map();
  const ttftByModel = new Map();
  for (const s of state.sessions.values()) {
    if (s.status === 'active' || !s.endedAt) continue;
    if (s.finalTps != null && s.finalTps > 0) {
      let arr = tpsByModel.get(s.model);
      if (!arr) { arr = []; tpsByModel.set(s.model, arr); }
      arr.push(s.finalTps);
    }
    if (s.firstTokenAt != null && s.startedAt != null) {
  ...
  const allTtft = [];
  const models = {};
  for (const model of new Set([...tpsByModel.keys(), ...ttftByModel.keys()])) {
    const tpsArr = tpsByModel.get(model) || [];
    const ttftArr = ttftByModel.get(model) || [];
    allTps.push(...tpsArr);
    allTtft.push(...ttftArr);
    models[model] = { tpsP50: pct(tpsArr, 0.5), ttftMsP50: pct(ttftArr, 0.5) };
  }
  return {
    overall: { tpsP50: pct(allTps, 0.5), ttftMsP50: pct(allTtft, 0.5) },
    models,
  };
}
