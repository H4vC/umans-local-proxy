...
// Fraction of burst headroom currently available. Exposed for telemetry only:
// local admission deliberately does NOT spend burst headroom because burst
// semantics are provider-side and can still 429 when a client releases a large
// parallel batch. The proxy gates to the base concurrency limit instead.
...
function concurrencyQuotaLimit(concurrency) {
  ...
  if (!soft) return null;
  return hard && hard < soft ? hard : soft;
}
...
async function doFetchUmansUsage() {
  try {
    ...
    state.concurrencyCache = {
      concurrent: Number(raw?.usage?.concurrent_sessions ?? raw?.usage?.concurrent ?? raw?.concurrent_sessions ?? 0) || 0,
...
function getEffectiveConcurrency() {
  ...
  // C3: while the usage cache is stale (last /usage fetch expired and the
  // background refresh hasn't succeeded — typically a /usage outage), the
  // cached upstream concurrent_sessions is stale-high. Prefer our own
  // activeRequests so admissions don't starve behind a figure that no longer
  // reflects reality.
