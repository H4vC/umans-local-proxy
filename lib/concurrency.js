...
function extractThrottle(data) {
  ...
  const limit = clampBoxedLimit(applied.limit, applied.softLimit, boxMs);
  return {
...
function getEffectiveConcurrency() {
  ...
  return { concurrent: state.concurrencyCache.concurrent || 0, limit: effectiveLimit, softLimit, overridden, boxed: boxedActive(state.concurrencyCache.boxedUntil), boxedUntil: state.concurrencyCache.boxedUntil ? new Date(state.concurrencyCache.boxedUntil).toISOString() : null };
}
