...
// relay (account-wide 429/ban risk). After the first success a null limit
// genuinely means "no cap" (C2).
...
// cooldown window from now, so repeated 429s keep bursting off. We do NOT wake
// queued waiters here: waking them under the freshly-clamped limit lets them
// immediately hit the (still-429'ing) upstream and slide the window again
// (cascade). Let in-flight requests drain; their releases wake the queue as
// real slots free (C5).
