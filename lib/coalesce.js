...
const DEFAULT_STATE_MAP_LIMIT = SCALING_DEFAULTS.stateMap;
const DEFAULT_MESSAGE_HASH_CACHE_LIMIT = SCALING_DEFAULTS.messageHashCache;
...
function messageHash(message) {
  ...
  const cacheLimit = configuredLimit('messageHashCache', DEFAULT_MESSAGE_HASH_CACHE_LIMIT);
  const cacheable = cacheLimit > 0 && json.length <= MAX_MESSAGE_HASH_CACHE_KEY_CHARS;
  if (cacheable) {
  ...
  state.messageHashCache.set(json, h);
  while (state.messageHashCache.size > cacheLimit) {
...
function storeStateKey(model, messages, responseContent, groupKey, prefixChain, prefixChainNum) {
  ...
  const stateMapLimit = configuredLimit('stateMap', DEFAULT_STATE_MAP_LIMIT);
  if (stateMapLimit <= 0) return;
  while (!state.stateMap.has(stateChain) && state.stateMap.size >= stateMapLimit) evictOldestStateKey();
...
const stateMapLimit = configuredLimit('stateMap', DEFAULT_STATE_MAP_LIMIT);
while (state.stateMap.size > stateMapLimit) evictOldestStateKey();
...
const messageHashCacheLimit = configuredLimit('messageHashCache', DEFAULT_MESSAGE_HASH_CACHE_LIMIT);
while (state.messageHashCache.size > messageHashCacheLimit) state.messageHashCache.delete(state.messageHashCache.keys().next().value);
...
module.exports = {
  ...
  storeStateKey,
  newSessionId,
  DEFAULT_STATE_MAP_LIMIT,
  DEFAULT_MESSAGE_HASH_CACHE_LIMIT,
  MAX_COALESCE_DEBUG_ENTRIES,
};
