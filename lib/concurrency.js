...
// admit the soft limit plus the available portion of burst headroom, but keep
// one slot below the upstream hard limit as a guardrail against edge-of-window
// races where the provider reports burst capacity that is exhausted by the
...
function concurrencyQuotaLimit(concurrency) {
  ...
  const burstCeiling = Math.max(soft, hard - 1);
