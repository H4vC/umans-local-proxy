...
const TAP_PENDING_BYTES_MAX = 256 * 1024; // per tap: includes unflushed + worker-owned batches
const GLOBAL_PENDING_BYTES_MAX = 4 * 1024 * 1024; // all worker-bound telemetry, shared across hot reloads
const MAX_TPS_SAMPLES = 128; // rolling live-rate samples retained per active session
  ...
  if (op === 'sync') {
    ...
    if (s && msg.fields) {
      ...
      if (typeof msg.fields.outputTokens === 'number') {
        ...
        if (h || samples.length > MAX_TPS_SAMPLES) {
          const trim = Math.max(h, samples.length - MAX_TPS_SAMPLES);
...
class ChatTap {
  constructor(session, { stream, onFinalize, shape = 'openai' }) {
    ...
    if (session && session.model) {
      ...
      postToWorker({ op: 'init', id: this.id, shape, model: session.model, stream, discardBuffer: this.discardSignal.buffer });
    }
  }

  reservePending(bytes) {
    if (bytes <= 0 || this.pendingBytes + bytes > TAP_PENDING_BYTES_MAX || state.tapPendingBytes + bytes > GLOBAL_PENDING_BYTES_MAX) return false;
