...
const SCALING_DEFAULTS = Object.freeze({
  sessionHistory: 500,
  groupSummaries: 500,
  seenModels: 128,
  modelRatios: 128,
  stateMap: 5000,
  messageHashCache: 5000,
  throttleWaiters: 8,
  bodyReads: 4,
  tapPendingBytes: 256 * 1024,
  globalTapPendingBytes: 4 * 1024 * 1024,
  tpsSamples: 128,
});

const SCALING_FIELDS = Object.freeze({
  sessionHistory: 'SESSION_HISTORY_LIMIT',
  groupSummaries: 'GROUP_SUMMARY_LIMIT',
  seenModels: 'SEEN_MODELS_LIMIT',
  modelRatios: 'MODEL_RATIO_LIMIT',
  stateMap: 'STATE_MAP_LIMIT',
  messageHashCache: 'MESSAGE_HASH_CACHE_LIMIT',
  throttleWaiters: 'THROTTLE_QUEUE_LIMIT',
  bodyReads: 'BODY_READ_LIMIT',
  tapPendingBytes: 'TAP_PENDING_BYTES',
  globalTapPendingBytes: 'GLOBAL_TAP_PENDING_BYTES',
  tpsSamples: 'TPS_SAMPLE_LIMIT',
});

function parseNonNegativeInt(value, fallback, label) {
  const raw = value == null || value === '' ? String(fallback) : String(value).trim();
...
function loadScaling(raw) {
  const limits = {};
  for (const [key, envName] of Object.entries(SCALING_FIELDS)) {
    const value = process.env[envName] !== undefined ? process.env[envName] : raw[envName];
    limits[key] = parseNonNegativeInt(value, SCALING_DEFAULTS[key], envName);
  }
  return limits;
}

function saveScaling(next) {
  const out = {};
  for (const [key, envName] of Object.entries(SCALING_FIELDS)) {
    out[envName] = parseNonNegativeInt(next?.[key], SCALING_DEFAULTS[key], envName);
  }
  return out;
}
...
function parseDuration(value, label = 'REQUEST_TIMEOUT') {
  const raw = String(value || '15m').trim().toLowerCase();
  const match = raw.match(/^(\d+)(ms|s|m|h)?$/);
  if (!match) throw new Error(`${label} must be like 30000ms, 30s, 15m, or 1h`);
...
function normalizeSessionCookie(raw) {
  ...
  if (/^[^\s=;]+=/.test(v)) header = v;
  else header = `${SESSION_COOKIE_NAME}=${v}`;
  // A Cookie header value must be a Latin-1 ByteString: undici throws a
  // TypeError on any character > 0xFF BEFORE the request is sent. The common
  // failure is a copy-paste artifact — a truncated DevTools/docs field renders
  // "…" as U+2026 (the ellipsis char), which lands mid-token. Every cap-health
  // fetch then throws, the 429 backoff gate's catch (concurrency.js) arms the
  // 72m cooldown unconditionally on every transient 429, and the user sees no
  // error: hasSessionCookie stays true. Cookie-octets are printable ASCII
  // (≤ 0x7E); reject anything outside [0x20,0x7E] (also catches NBSP 0xA0,
  // smart quotes, em-dashes, lone surrogates), drop the cookie so cap-health
  // fails clean (legacy fail-safe, no fetch spam) and tell the user what broke.
  for (let i = 0; i < header.length; i++) {
    const code = header.charCodeAt(i);
    if (code < 0x20 || code > 0x7E) {
      const hex = code.toString(16).toUpperCase().padStart(4, '0');
      console.warn(
        `[config] SESSION_COOKIE discarded: non-ASCII character U+${hex} at position ${i} ` +
        `(common cause: copying the token from a truncated field that inserted an ellipsis "…" or smart quote). ` +
        `cap-health is disabled and the 429 gate falls back to the legacy fail-safe until a valid token is pasted. ` +
        `Re-copy the raw __Secure-authjs.session-token value from DevTools (Application → Cookies → app.umans.ai).`
      );
      return '';
    }
  }
...
function loadConfig() {
  ...
  const releaseCooldownRaw = process.env.RELEASE_COOLDOWN_MS || raw.RELEASE_COOLDOWN_MS || '1s';
  return {
    ...
    requestTimeoutRaw,
    releaseCooldownMs: parseDuration(releaseCooldownRaw, 'RELEASE_COOLDOWN_MS'),
    releaseCooldownRaw,
    requestLogging,
...
function saveConfig(next) {
  ...
  fs.writeFileSync(CONFIG_FILE, JSON.stringify({
    ...
    RELEASE_COOLDOWN_MS: next.releaseCooldownRaw,
    WEBSEARCH_PROVIDER: next.websearchProvider,
    ...saveScaling(next.limits),
  }, null, 2) + '\n', { mode: 0o600 });
