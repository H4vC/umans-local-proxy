...
const MAX_QUEUE_BYTES = 512 * 1024; // slow clients retain bounded dashboard telemetry
...
function ensureQueueAccounting(client) {
  if (!Array.isArray(client.queue)) client.queue = [];
  if (!Number.isFinite(client.queueBytes) || client.queueBytes < 0) {
    client.queueBytes = client.queue.reduce((total, frame) => total + (frame?.length || 0), 0);
  }
  if (!Array.isArray(client.queueTypes) || client.queueTypes.length !== client.queue.length) {
    client.queueTypes = new Array(client.queue.length).fill(null);
  }
}

function payloadType(payload) {
  const match = typeof payload === 'string' && /^\{"type":"([^"\\]+)"/.exec(payload);
  return match ? match[1] : null;
}

function removeQueuedAt(client, index) {
  const frame = client.queue[index];
  client.queue.splice(index, 1);
  client.queueTypes.splice(index, 1);
  client.queueBytes = Math.max(0, client.queueBytes - (frame?.length || 0));
}

function queueFrame(client, frame, type) {
  ensureQueueAccounting(client);
  if (frame.length > MAX_QUEUE_BYTES) return; // an individual dashboard update is optional telemetry
  if (type === 'sessions') {
    // A snapshot supersedes every queued event: retaining only the newest view
    // makes a slow dashboard converge while keeping memory byte-bounded.
    client.queue.length = 0;
    client.queueTypes.length = 0;
    client.queueBytes = 0;
    client.queue.push(frame);
    client.queueTypes.push(type);
    client.queueBytes = frame.length;
    return;
  }
  // Preserve an already queued latest snapshot. Under pressure, drop older
  // non-snapshot telemetry first; the incoming event is dropped if it cannot
  // fit without making the client regress to an older dashboard state.
  while (client.queueBytes + frame.length > MAX_QUEUE_BYTES) {
    const index = client.queueTypes.findIndex((queuedType) => queuedType !== 'sessions');
    if (index < 0) return;
    removeQueuedAt(client, index);
  }
  client.queue.push(frame);
  client.queueTypes.push(type);
  client.queueBytes += frame.length;
...
function addClient(socket) {
  const client = { socket, queue: [], queueTypes: [], queueBytes: 0, paused: false, alive: true, awaitingPong: false };
  ...
  socket.on('drain', () => {
    ...
    while (client.queue.length > 0) {
      ...
      client.queueTypes.shift();
      client.queueBytes = Math.max(0, client.queueBytes - frame.length);
...
function send(client, payload) {
  ...
  if (client.paused || client.queue.length > 0) {
    queueFrame(client, frame, payloadType(payload));
    return;
  }
  try {
