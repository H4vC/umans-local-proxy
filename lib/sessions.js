...
function updateGroupSummary(session) {
  ...
  }, state.GROUP_SUMMARY_TTL_MS).unref();
}
...
function finalizeSession(session, status) {
  ...
  setTimeout(() => { state.sessions.delete(session.id); scheduleSessionsBroadcast(); }, 120000);
  ...
  for (const res of state.sseClients) {
    try {
      if (!res.write(payload)) {
        // Client buffer full (backpressure) — disconnect slow consumer.
